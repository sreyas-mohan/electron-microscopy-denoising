...
def add_logging_arguments(parser):
    ...
    parser.add_argument(
        "--output-dir",
        default="experiments",
        help="path to experiment directories")
    parser.add_argument(
        "--experiment",
        default=None,
        help="experiment name to be used with Tensorboard")
    parser.add_argument(
        "--resume-training",
        action="store_true",
    ...
    parser.add_argument(
        "--step-checkpoints",
        action="store_true",
        help="store all step checkpoints")
    parser.add_argument(
        "--no-log",
        action="store_true",
        help="don't save logs to file or Tensorboard directory")
    parser.add_argument(
        "--log-interval",
        type=int,
        default=100,
    ...
    parser.add_argument(
        "--draft",
        action="store_true",
        help="save experiment results to draft directory")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="no log, no save, no visualization")
    return parser


def setup_experiment(args):
    def contrast_name(contrast):
    ...
    # change folder name so that we can easily distinguish
    filename_array = [args.experiment,
                                'BF' if (not args.bias) else 'B',
                                str(args.scales) if (args.model == 'unet2') else None,
                                str(args.base_channels) if (args.model == 'unet2') else None,
#                                 'R' if args.residual else None,
                                str(int(args.noise_scale)),
                                args.allowed_gen_values if args.allowed_gen_values != 'None' else None]
    filename_array = [x for x in filename_array if x is not None]
    args.experiment = "-".join(filename_array)
    ...
    args.experiment_dir = os.path.join(
        args.output_dir,
        f"{args.dataset}_{contrast_name(args.contrast)}",
        (f"drafts/" if args.draft else "") +
        args.experiment)
...
def save_checkpoint(
    ...
    if (score < best_score and mode == "min") or (
            score > best_score and mode == "max"):
        save_checkpoint.best_step = step
        save_checkpoint.best_score = score

    if not args.no_save and step % args.save_interval == 0:
        os.makedirs(args.checkpoint_dir, exist_ok=True)
        model = [model] if model is not None and not isinstance(
            model, list) else model
...
def load_checkpoint(args, model=None, optimizer=None, scheduler=None):
    if args.restore_file is not None and os.path.isfile(args.restore_file):
        ...
        if optimizer is not None and state_dict.get(
                "optimizer", None) is not None:
            for o, state in zip(optimizer, state_dict["optimizer"]):
                o.load_state_dict(state)
        if scheduler is not None and state_dict.get(
                "scheduler", None) is not None:
