# From https://github.com/LabForComputationalVision/bias_free_denoising/blob/master/models/BFBatchNorm2d.py
        ...
        if self.training is not True:
            ...
            if self.track_running_stats is True:
                with torch.no_grad():
                    ...
                    self.running_var = (
                        1 - self.momentum) * self.running_var + self.momentum * sigma2
            if self.use_bias:
                y = y - mu.view(-1, 1)
...
def unit_test():
    ...
    for _ in range(25):
        ...
        print("bias: variance %f, mean %f" %
              (torch.var(bias_out), torch.mean(bias_out)))
        print_bn_details(bn_bias)

        bf_out = bn_bf(temp_inp)
        print("bf: variance %f, mean %f" %
              (torch.var(bf_out), torch.mean(bf_out)))
